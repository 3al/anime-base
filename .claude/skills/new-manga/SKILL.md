...
**Adult-ветка (`content_rating ∈ {nsfw, explicit}`):** обложку берём **по провенансу** — id-locked CDN (AniList `Media(type:MANGA).coverImage.extraLarge` по id), валидируем программно (тип/размер/сигнатура), вшиваем `![[<file>|300]]` + `images.cover` **без probe и без визуального Read** (см. `CLAUDE.md` → «Adult-контент: укладка по провенансу»). Probe/gate ниже — **только для `sfw`**. Жёсткий предел (несовершеннолетние) — subject-based, на сборе данных, не здесь.
