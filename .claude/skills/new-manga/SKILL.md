...
- **MyAnimeList** (`myanimelist.net/manga/<id>`) — volumes, chapters, status, published dates, demographic, serialization, authors, score.
...
last_chapter_read:             # опционально, USER-ONLY — прогресс чтения
images:
...
**`USER-ONLY`-поля** (`personal_status`, `personal_score`, `art_score`, `story_score`, `originality_score`, `times_read`, `last_chapter_read`) — оставить **пустыми**, не выдумывать. Рендерятся всегда (discoverability), НЕ удаляются даже если пусты. **`translation_status`** — тоже user-filled (модель не знает состояние перевода/скана) → default пусто, из веба не выводить. **`content_rating`/`coloring`** — модель-заполняемые (см. шаг 2).

**Теги подбирать пошагово** (как `/new-note` шаг 6). У `note_kind: manga` **обязательных тегов нет** — категоризация закрыта полями (`demographic`, `genres`, `status`). Не дублировать enum-поля в теги. Новый тег не из `Tag_taxonomy.md` — добавить туда.
...
<Проза: автор(ы), журнал, период серализации, контекст создания. Имена в прозе — plain text. Затем gallery-список авторов (`PERSONS/`, ширина 60):>
...
- [ ] `USER-ONLY`-поля (`personal_status`, `personal_score`, `times_read`, `last_chapter_read`) и `## Личный отзыв` — НЕ выдуманы, оставлены пустыми.
