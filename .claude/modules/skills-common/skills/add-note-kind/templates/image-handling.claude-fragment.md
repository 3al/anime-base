...
3. **Расширение = реальный тип содержимого.** Расширение файла берётся из **фактического** типа (по результату `file` / Content-Type), а **не** хардкодится. Скачанный как `*.png` файл, внутри которого `file` показывает JPEG/WebP, — переименовать под реальный тип (`.jpg` / `.webp`) до записи `images:` и вставки в галерею. Иначе путь во frontmatter и эмбеде врёт о типе, а часть рендереров/инструментов спотыкается.

4. **НИКОГДА** не отправлять файл на визуальную оценку (Read tool) без прохождения проверок 1–3. Битое изображение отравляет контекст модели и вызывает ошибку `"Could not process image"` на все последующие запросы в сессии.

### Именование ассета и anti-clobber

**Имя ассета:** `<basename_заметки>_<роль>.<ext>` — `role` различает типы (`cover`, `top`, `bottom`, `habitat` и т.п.), `ext` совпадает с реальным типом содержимого (см. валидацию п. 3). Это удерживает уникальность имени и привязку к заметке.

**Никогда не перезаписывать чужой файл молча.** Перед сохранением скачанного/скопированного ассета проверить, **не занят ли целевой путь** (существует ли уже файл с таким именем в `attachments/`). Если занят:
- путь принадлежит **этой же** заметке и роли (пере-скачивание своего ассета) → перезапись допустима;
- путь принадлежит **другой** заметке (то же имя, другой объект) → **СТОП, не перезаписывать**. Дизамбигуировать имя нового ассета (добавить различающий суффикс/квалификатор) и сохранить под ним. Тихая перезапись чужой обложки — потеря данных.

### Вставка изображения в заметку (потолок ширины)

- **Cover / постер** (одиночный главный эмбед) — кэпить шириной: `![[<файл>|300]]`. Потолок `|300`; конкретное значение может задать тематическая конвенция волта (`SYSTEM/Linking_guidelines.md`), но **не растягивать эмбед без `|N`** — Obsidian развернёт его в полный размер.
- **Галерейные миниатюры** — узкие по конвенции волта (обычно 60–120), не трогать этим правилом.
- Любой `![[...]]` без `|N` для cover/постера — дефект: проставить ширину.
