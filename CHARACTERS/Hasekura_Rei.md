---
type: reference
domain: anime
stability: stable
priority: medium
quality: draft
co_authored: claude-opus-4.8
note_kind: character
tags:
  - shoujo
  - coming-of-age
aliases:
  - Рэй Хасэкура
  - Rei Hasekura
created: 2026-05-30
updated: 2026-06-10
name_romaji: "Hasekura Rei"
name_native: "支倉令"
name_cyrillic: "Рэй Хасэкура"
name_english: "Rei Hasekura"
featured_in:
  - Maria-sama_ga_Miteru
  - Maria-sama_ga_Miteru_Haru
  - Maria-sama_ga_Miteru_3rd
  - Maria-sama_ga_Miteru_4th
  - Maria-sama_ga_Miteru_(manga)
character_role: supporting
gender: female
status: alive
age: "16-17"
species: human
affiliations:
  - Лилианская женская академия
  - Совет Ямаюрикай
  - Семья Хасэкура
voice_actors: []
mal_url: "https://myanimelist.net/character/1628/Rei_Hasekura"
anilist_url: "https://anilist.co/character/1628/Rei-Hasekura"
images:
  cover: "attachments/Hasekura_Rei_cover.jpg"
personal_score: null
---

# Hasekura Rei (支倉令)

![[Hasekura_Rei_cover.jpg|300]]

**Рэй Хасэкура** — член Совета Ямаюрикай католической женской академии Лилиан, носительница титула Rosa Foetida en bouton (позднее — Rosa Foetida) в сериале «Мария-сама смотрит за нами». Старшая сестра Ёсино Симадзу, внешне — мальчишеская бисёнэн, внутри — нежная и домашняя.

## Описание

Высокая, коротко стриженная блондинка с карими глазами; в школьной форме держится по-мужски уверенно, за что получила школьный титул «Мистер Лилиан». Вопреки бисёнэн-внешности, натура у Рэй на редкость женственная: она обожает стряпать, печь, шить (шарф и варежки Ёсино — её работа) и читать сёдзё-романы. Любимое слово — «искренность», хобби — туризм. Участница клуба кэндо, но к спортплощадке равнодушна: настоящая страсть — забота о Ёсино.

## Биография

> [!warning]- Спойлеры (аниме)
>
> **Первый сезон.** Рэй — второкурсница и Rosa Foetida en bouton, младшая сестра Эрико Тории. Её собственная младшая сестра — двоюродная сестра Ёсино Симадзу, с которой они росли вместе. Рэй опекает Ёсино с навязчивой преданностью, забывая о себе: во время турнира по кэндо думает не о поединке, а о том, как Ёсино. Когда газетный клуб публикует анкету с перепутанными ответами Рэй и Ёсино (газетчицы приняли женские хобби Рэй за ответы Ёсино и наоборот), Рэй спокойно принимает недоразумение, тогда как Ёсино бурно реагирует.
>
> **Второй сезон (*Haru*).** Рэй вместе с Ёко и Сэй выпускается из Лилиан. Передаёт титул бутону — но лента Rosa Foetida пока не передана конкретной преемнице. Выпускная церемония завершает её эру бутона, а затем и полноценной розы.
>
> **Третий сезон (*3rd*).** Ёсино, оправившись после операции на сердце, выставляет кандидатуру на выборах в Совет — частично чтобы доказать независимость от Рэй. Рэй болезненно переживает, что Ёсино хочет отдалиться, но постепенно принимает её право на самостоятельность.

## Тайтлы

- ![[Maria-sama_ga_Miteru_cover.jpg|80]] **[[Maria-sama_ga_Miteru|Maria-sama ga Miteru]]** — 2004, school-драма с элементами class-S; Rosa Foetida en bouton, опекунша Ёсино.
- ![[Maria-sama_ga_Miteru_Haru_cover.jpg|80]] **[[Maria-sama_ga_Miteru_Haru|Maria-sama ga Miteru: Haru]]** — 2004, второй сезон; выпуск из Лилиан и передача титула Rosa Foetida.
- ![[Maria-sama_ga_Miteru_3rd_cover.jpg|80]] **[[Maria-sama_ga_Miteru_3rd|Maria-sama ga Miteru 3rd]]** — 2006, третий сезон (OVA); реакция на выборную кампанию Ёсино.
- ![[Maria-sama_ga_Miteru_4th_cover.jpg|80]] **[[Maria-sama_ga_Miteru_4th|Maria-sama ga Miteru 4th]]** — 2009, четвёртый сезон; отпускает повзрослевшую Ёсино, ставшую Rosa Foetida en bouton.
- ![[Maria-sama_ga_Miteru_(manga)_cover.jpg|80]] **[[Maria-sama_ga_Miteru_(manga)|Maria-sama ga Miteru (манга)]]** — 2003–2007, манга-адаптация Сатору Нагасавы; Rosa Foetida en bouton, опекунша Ёсино.

## Связи

### В каноне

- ![[Yoshino_Shimazu_cover.jpg|60]] **[[Yoshino_Shimazu|Ёсино Симадзу]]** — младшая сестра (petite soeur) и двоюродная сестра; центр привязанности Рэй, её навязчивая забота — одновременно источник тепла и напряжения в их отношениях.
- ![[Torii_Eriko_cover.jpg|60]] **[[Torii_Eriko|Эрико Тории]]** — старшая сестра (grande soeur); выбрала Рэй ради эксперимента, но связь оказалась глубже, чем предполагалось.
- ![[Fukuzawa_Yumi_cover.jpg|60]] **[[Fukuzawa_Yumi|Юми Фукудзава]]** — коллега по Совету Ямаюрикай; Рэй относится к ней мягко, как и ко всем окружающим.
- ![[Ogasawara_Sachiko_cover.jpg|60]] **[[Ogasawara_Sachiko|Сатико Огасавара]]** — Rosa Chinensis en bouton, коллега по Совету.
- ![[Sato_Sei_cover.jpg|60]] **[[Sato_Sei|Сэй Сато]]** — Rosa Gigantea, одна из старших Роз; единственная, кто может сравниться с Рэй в «мальчишеском» имидже.
- ![[Mizuno_Yoko_cover.jpg|60]] **[[Mizuno_Yoko|Ёко Мидзуно]]** — Rosa Chinensis, лидер выпускного триумвирата; к ней Рэй обращается за советом в спорных ситуациях.
- ![[Toudou_Shimako_cover.jpg|60]] **[[Toudou_Shimako|Симако Тодо]]** — Rosa Gigantea en bouton, коллега по Совету одного поколения.

## Сэйю

- **Shizuka Itō (伊藤 静)** — японская озвучка, все тайтлы.
- **Candice Moore** — английский дубляж (Maria-sama ga Miteru).

## Ссылки

- [MyAnimeList](https://myanimelist.net/character/1628/Rei_Hasekura)
- [AniList](https://anilist.co/character/1628/Rei-Hasekura)
- [Fandom (Marimite wiki)](https://marimite.fandom.com/wiki/Rei_Hasekura)
