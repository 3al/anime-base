...
**title_cyrillic** — название кириллицей (**обязательное**): общеупотребительная русская локализация, если она устоялась (`Клинок, рассекающий демонов`, `Унесённые призраками`); если устоявшегося варианта нет — создающий скилл (`/new-anime`) **предлагает пользователю несколько вариантов перевода структурированным multi-choice вопросом и просит выбрать либо указать свой** (можно оставить свой вариант). Молча выдумывать/«лучший-угаданный» перевод запрещено — в отличие от персонажей/персон, где кириллизация это однозначная транскрипция, для тайтла перевод — решение пользователя. Аудит проверяет заполненность (пустое → дефект; для карточек старше ввода поля — `SCHEMA_DRIFT`, vault-долг на бэкафилл).

```yaml
title_cyrillic: "Клинок, рассекающий демонов"
```
...
**name_cyrillic** — имя кириллицей (**обязательное**): общеупотребительный русский вариант, если он устоялся (`Хаяо Миядзаки`); если устоявшегося нет — практическая транскрипция с языка оригинала. Создающий скилл (`/new-person`) заполняет молча — кириллизация имени однозначна, **без** вопроса пользователю (в отличие от тайтлов). Аудит проверяет заполненность (пустое → дефект; для карточек старше ввода поля — `SCHEMA_DRIFT`).
...
**name_cyrillic** — имя кириллицей (**обязательное**): общеупотребительный русский вариант, если он устоялся (`Танджиро Камадо`, `Вайолет Эвергарден`, `Гилберт Бугенвиллея`); если устоявшегося нет — практическая транскрипция с языка оригинала (`Куротова`, `Цутако Такэсима`). Создающий скилл (`/new-character`) заполняет молча — для персонажей кириллизация однозначна, **без** вопроса пользователю (в отличие от тайтлов `anime`/`manga`). Аудит проверяет заполненность (пустое → дефект; для карточек старше ввода поля — `SCHEMA_DRIFT`).

```yaml
name_cyrillic: "Танджиро Камадо"
```
...
**title_cyrillic** — название кириллицей (**обязательное**): общеупотребительная русская локализация, если она устоялась; если устоявшегося варианта нет — создающий скилл (`/new-manga`) **предлагает пользователю несколько вариантов перевода структурированным multi-choice вопросом и просит выбрать либо указать свой**. Молча выдумывать перевод запрещено (как у `anime.title_cyrillic`). Аудит проверяет заполненность (пустое → дефект; для карточек старше ввода поля — `SCHEMA_DRIFT`).

```yaml
title_cyrillic: "Берсерк"
```
